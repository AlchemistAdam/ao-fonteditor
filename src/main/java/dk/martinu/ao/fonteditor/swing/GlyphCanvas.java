...
//    public static final String PROPERTY_DIRTY = "dk.martinu.ao.fonteditor.swing.GlyphCanvas.DIRTY";
     ...
     */
    protected final EditorFrame editor;
    /**
     * The glyph this data represents.
     */
    @NotNull
    protected final MutableGlyph glyph;
    /**
...
//    protected boolean isDirty = false;
     ...
     */
    public GlyphCanvas(@NotNull final EditorFrame editor, @NotNull final MutableGlyph glyph) {
...
//    }
    ...
    public void propertyChange(@NotNull final PropertyChangeEvent event) {
        switch (event.getPropertyName()) {
            ...
            case PROPERTY_TOOL_COLOR -> setToolColor((Color) event.getNewValue());
        }
    }
...
//    }
     ...
     */
    @Contract(mutates = "this")
    public void setToolColor(@NotNull final Color color) {
    ...
    protected void paintComponent(@NotNull final Graphics g) {
        ...
        // half canvas size
        final int hWidth = getWidth() / 2;
        final int hHeight = getHeight() / 2;
        // scaled glyph image bounds
        final int scaledX = (x - hWidth) * zoom + hWidth;
        final int scaledY = (y - hHeight) * zoom + hHeight;
        final int scaledWidth = image.getWidth() * zoom;
        final int scaledHeight = image.getHeight() * zoom;
     ...
     * repainted.
     */
    protected void renderGlyphToImage() {
        final WritableRaster raster = image.getRaster();
        if (!glyph.isWhitespace) {
            final int[] rgba = Arrays.copyOf(this.rgba, 4);
     ...
     */
    protected void saveImageToData() {
        final int width = glyph.width;
        final int height = glyph.height;
        final int len = width * height;
        final int[] samples = image.getRaster().getSamples(0, 0, width, height, 3, new int[len]);
        for (int i = 0; i < len; i++)
            glyph.data[i] = (byte) samples[i];
    }
    ...
    protected void updateImage() {
        final BufferedImage image = new BufferedImage(glyph.width, glyph.height, BufferedImage.TYPE_INT_ARGB);
        final int width = Math.min(glyph.width, this.image.getWidth());
        final int height = Math.min(glyph.height, this.image.getHeight());
    ...
    protected class EventHandler implements MouseListener, MouseMotionListener, MouseWheelListener {
         ...
         */
        @Nullable
        protected MouseEvent startEvent = null;
        /**
        ...
        public void mouseDragged(@NotNull final MouseEvent event) {
            ...
            if (tool == Tool.MOVE) {
                ...
                int newY = startY;
                if (zoom == 1) {
                    newX += event.getX() - startEvent.getX();
                    newY += event.getY() - startEvent.getY();
                }
                else {
                    newX += Math.round((float) (event.getX() - startEvent.getX()) / (float) zoom);
                    newY += Math.round((float) (event.getY() - startEvent.getY()) / (float) zoom);
                }
                // ensure newX stays within bounds
                final int hWidth = getWidth() / 2;
                final int scaledWidth = image.getWidth() * zoom;
                if (scaledWidth >= getWidth()) {
                    if (newX + image.getWidth() < hWidth) {
                        newX = hWidth - image.getWidth();
                    }
                    else if (newX > hWidth) {
                        newX = hWidth;
                    }
                    ...
                    final int scaledCenterX = (newX - hWidth) * zoom + hWidth + scaledWidth / 2;
                    if (scaledCenterX < 0) {
                        newX = (-scaledWidth / 2 - hWidth) / zoom + hWidth;
                    }
                    else if (scaledCenterX > getWidth()) {
                        newX = (hWidth - scaledWidth / 2) / zoom + hWidth;
                    }
                }
                ...
                if (scaledHeight >= getHeight()) {
                    if (newY + image.getHeight() < hHeight) {
                        newY = hHeight - image.getHeight();
                    }
                    else if (newY > hHeight) {
                        newY = hHeight;
                    }
                }
                else {
                    final int scaledCenterY = (newY - hHeight) * zoom + hHeight + scaledHeight / 2;
                    if (scaledCenterY < 0) {
                        newY = (-scaledHeight / 2 - hHeight) / zoom + hHeight;
                    }
                    else if (scaledCenterY > getHeight()) {
                        newY = (hHeight - scaledHeight / 2) / zoom + hHeight;
                    }
                }
                ...
                paintPixel(event.getX(), event.getY());
            else if (tool == Tool.ERASER)
                erasePixel(event.getX(), event.getY());
            else if (tool == Tool.PICKER)
                copyPixel(event.getX(), event.getY());
        }
        ...
        public void mouseWheelMoved(@NotNull final MouseWheelEvent event) {
            ...
            if (rotation < 0) // scroll up
                zoomIn();
            else if (rotation > 0) // scroll down
                zoomOut();
        }
         ...
         */
        protected void copyPixel(final int mouseX, final int mouseY) {
            final int[] pixel = getPixel(mouseX, mouseY, pixelBuffer);
            if (pixel[0] != -1) {
                final WritableRaster raster = image.getRaster();
                final int alpha = raster.getSample(pixel[0], pixel[1], 3);
                if (alpha != rgba[3])
         ...
         * @param mouseY y coordinate of the mouse
         */
        protected void erasePixel(final int mouseX, final int mouseY) {
            final int[] pixel = getPixel(mouseX, mouseY, pixelBuffer);
            if (pixel[0] != -1) {
                final WritableRaster raster = image.getRaster();
...
//                    setDirty(true);
        ...
        @Contract(pure = true)
        protected int[] getDiagonalPixels(int x0, int y0, int x1, int y1) {
            // half canvas size
            final int hWidth = getWidth() / 2;
            final int hHeight = getHeight() / 2;
            // scaled glyph image bounds
            ...
            final double slope = (double) (y1 - y0) / (double) (x1 - x0);
            final double intercept = y0 - slope * x0;
...
//                final int pxs = zoom * UPSCALE_FACTOR;
//                final int pxs_2 = pxs / 2;
                ...
                if (slope > 0) {
                    if (slope < 1) {
                        ...
                        final double dx = (y0 + (zoom * UPSCALE_FACTOR / 2d) - intercept * UPSCALE_FACTOR) / slope - x0;
         ...
         */
        @Contract(pure = true)
        protected int[] getHorizontalPixels(final int mouseY, int x0, int x1) {
            // half canvas size
            final int hWidth = getWidth() / 2;
            final int hHeight = getHeight() / 2;
            // scaled glyph image bounds
        ...
        protected int[] getPixel(final int mouseX, final int mouseY, final int[] pixel) {
            // half canvas size
            final int hWidth = getWidth() / 2;
            final int hHeight = getHeight() / 2;
            // scaled glyph image bounds
         ...
         */
        @Contract(pure = true)
        protected int[] getPixels(int x0, int y0, int x1, int y1) {
            if (x0 == x1)
                return getVerticalPixels(x0, y0, y1);
            else if (y0 == y1)
                return getHorizontalPixels(y0, x0, x1);
            else
                return getDiagonalPixels(x0, y0, x1, y1);
        }
         ...
         */
        @Contract(pure = true)
        protected int[] getVerticalPixels(final int mouseX, int y0, int y1) {
            // half canvas size
            final int hWidth = getWidth() / 2;
            final int hHeight = getHeight() / 2;
            // scaled glyph image bounds
            ...
            if (mouseX >= scaledX && mouseX < scaledX + scaledWidth) {
                ...
                if (y1 >= scaledY && y0 < scaledY + scaledHeight) {
                    ...
                    if (y1 >= scaledY + scaledHeight)
                        y1 = scaledY + scaledHeight - 1;
                    // ensure line segment spans more than a single pixel
                    final int p0 = y0 / zoom;
                    final int p1 = y1 / zoom;
                    if (p0 == p1)
                        return null;
                    // array length is multiplied with 3 because it will be
                    // reused later to include the pixels' color components
                    ...
                    // fill and return pixel coordinates
                    final int[] pixels = new int[len];
                    final int x = (mouseX - scaledX) / zoom;
                    for (int i = 0, y = (y0 - scaledY) / zoom + 1; i < len; i += 3, y++) {
                        pixels[i] = x;
                        pixels[i + 1] = y;
                    }
                    return pixels;
         ...
         * @param mouseY y coordinate of the mouse
         */
        protected void paintPixel(final int mouseX, final int mouseY) {
            final int[] pixel = getPixel(mouseX, mouseY, pixelBuffer);
            if (pixel[0] != -1) {
                // FIX
//                final int oldColor
...
//                    setDirty(true);
    ...
    protected static class RasterEdit extends ValueEdit<WritableRaster, int[]> implements BiConsumer<WritableRaster, int[]> {

        public RasterEdit(@NotNull final WritableRaster parent, final int[] rgba, final int[] pixels) {
            // FIX
...
//            this.newColor = newColor;
        ...
        @Override
        public void accept(final WritableRaster writableRaster, final int[] ints) {
